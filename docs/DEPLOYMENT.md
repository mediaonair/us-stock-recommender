...
- **LLM 분석 대상 뉴스 개수 제한**: 비용 절감을 위해 한 번 갱신할 때 전체 수집된 뉴스 중
  앞에서부터 `ANALYZE_LIMIT`(`src/workflow.py`, 기본값 15)건만 실제로 LLM 분석을 거칩니다.
  값이 너무 작으면 관심 종목이 여러 개일 때 일부 종목은 분석 대상에 들지 못해 추천/대시보드에
  아예 나타나지 않을 수 있습니다. 관심 종목 수가 늘어나면 이 값도 함께 늘리는 것을 권장합니다.
